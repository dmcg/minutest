...
import uk.org.minutest.RootDescriptor
...
interface TestExecutor<F> : TestDescriptor {
    ...
    fun runTest(test: uk.org.minutest.Test<F>) {
        runTest(test, this.andThenJust(test.name))
    }

    fun runTest(testlet: Testlet<F>, testDescriptor: TestDescriptor)

    fun <G> andThen(nextContext: uk.org.minutest.Context<F, G>): TestExecutor<G> = object: TestExecutor<G> {
